...
public class StooqClient {
    ...
    public String downloadDailyCsv(String symbol, LocalDate from, LocalDate to) throws IOException, InterruptedException {
        ...
        String url = baseUrl + "/q/d/l/?s=" + encodeSymbol(s)
                + "&d1=" + fmt.format(from)
                + "&d2=" + fmt.format(to)
                + "&i=d";
        ...
        if (resp.statusCode() != 200) {
            throw new IOException("Stooq CSV download failed: status=" + resp.statusCode() + " url=" + url);
        }
        return resp.body();
    }
